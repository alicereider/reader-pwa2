...
export default function ReaderToolbar({ title, onToggleSettings, onBookmark, bookId }) {
  return (
    <div className="reader-toolbar glass-card">
      <Link to={`/book/${bookId}`} className="icon-button icon-button--solid" aria-label={t.reader.backToBook}>
        <ArrowLeft size={18} />
      </Link>
